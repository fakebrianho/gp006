...
export default function Particles() {
	...
	bGeometry.setAttribute('uv', new THREE.BufferAttribute(uvs, 2))
	...
	const NewMaterial = shaderMaterial(
		{
			time: { value: 0 },
			uTexture: { value: new TextureLoader().load('/test.jpg') },
		...
		void main() {
			...
			// newpos.xy = color.xy;
			// newpos.z += sin( time + position.x*10. ) * 0.05;
			// newpos.z += sin(time);
			// newpos.z += sin(time);
			...
			// gl_Position = projectionMatrix * modelViewMatrix * color;
			vUv = uv;
		...
		void main() {
			...
			gl_FragColor = vec4(1.0, sin(time), 1.0, 1.0);
			// gl_FragColor = c
		}
		`
	)
...
`
	...
	const TextureMaterial = shaderMaterial(
		{ uTexture: new TextureLoader().load('/1.png') },
		vertexShader,
		fragmentShader
	)
